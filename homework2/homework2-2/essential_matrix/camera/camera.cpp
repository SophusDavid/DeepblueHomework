...
void Camera::GenerateFrames() {
    ...
    for (float t = t_start; t< t_end;) {
        ...
        for (int i = 0; i < ModelPoints.size(); ++i) {
            ...
            std::string text = std::to_string(int(proj(0))) + ", " + std::to_string(int(proj(1)));
            cv::putText(cur_image, text, cv::Point(int(proj(0)), int(proj(1))), cv::FONT_HERSHEY_SIMPLEX, 0.5, cv::Scalar(0, 5, 255), 1, 8, false );
